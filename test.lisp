...
(define-function (prop-runner props thunk)
  (let ((r (test-runner-null))
    ...
    ;; that is not in the property alist will occur as 'NIL
    (map (lambda (k)
           (assq k plist))
         props)))
...
(define-function (on-test-runner thunk visit)
  (let ((r (test-runner-null))
        (results '()))
    ;;
    (test-runner-on-test-end!
     ...
     (lambda (runner)
       ...
       (set! results (cons (funcall visit r) results))))
    ;;
    (test-with-runner r (funcall thunk))
    (reverse results)))
...
(define-function (triv-runner thunk)
  (let ((r (test-runner-null))
        (accum-pass '())
        (accum-fail '())
        (accum-xfail '())
    ...
    (test-runner-on-bad-count!
     ...
     (lambda (runner count expected-count)
       runner
       (error (string-append "bad count " (princ-to-string count)
			     " but expected "
			     (princ-to-string expected-count)))))
...
(define-function (path-revealing-runner thunk)
  (let ((r (test-runner-null))
        (seq '()))
    ;;
    (test-runner-on-test-end!
...
(test-equal
 ...
 '(("a") ("") () () () (1 1 0 0 0))
 (triv-runner (lambda () (test-assert "a" 'T) (test-assert 'NIL))))
...
(define-function (choke)
  (eval '(vector-ref '#(1 2) 3)))
...
(test-equal
 ...
 (triv-runner
  (lambda ()
    ...
    ;;                        ; #   "y"  2   result
    (test-assert "x" 'T)      ; 1 - 'NIL   'NIL  PASS
    (test-assert "y" 'NIL)      ; 2 - 'T   'T  SKIP
    (test-assert "y" 'NIL)      ; 3 - 'T   'NIL  FAIL
    (test-assert "x" 'NIL)      ; 4 - 'NIL   'NIL  FAIL
    (test-assert "z" 'NIL)      ; 5 - 'NIL   'NIL  FAIL
    (test-end))))
...
(test-equal
 ...
 (triv-runner
  (lambda ()
    ...
    (test-assert "x" 'T)      ; 1 - 'NIL   'NIL  PASS
    (test-assert "y" 'NIL)      ; 2 - 'T   'T  SKIP
    (test-assert "y" 'NIL)      ; 3 - 'T   'NIL  SKIP
    (test-assert "x" 'NIL)      ; 4 - 'NIL   'NIL  FAIL
    (test-assert "z" 'NIL)      ; 5 - 'NIL   'NIL  FAIL
    (test-end))))
              ...
              (lambda ()
                ...
                (test-with-runner
                 ...
                 (lambda ()
                   ...
                   (test-assert (eval '(vector-ref '#(3) 10)))  ; fail with error
                   (test-end "a")))
             ...
             (lambda ()
               ...
               (test-apply
                ...
                (lambda ()
                  ...
                  (test-assert "x" 'NIL)  ; # 4 SKIP=yes APPLY=no
                  0))
               (test-assert "v" 'T))))
...
(define-function (auxtrack-runner thunk)
  (let ((r (test-runner-null)))
...
(define-function (symbol-alist? l)
  (if (null? l)
      'T
      (and (pair? l)
           (pair? (car l))
